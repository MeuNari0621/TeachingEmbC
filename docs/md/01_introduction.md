...
本ドキュメント群は、一般論から具体例へ進む順で読みやすく再構成しています。まず「なぜテストしにくいのか」を整理し、その次に「どう境界を設計するか」を学び、最後に状態遷移や割り込みの具体例へ進みます。

```mermaid
graph TD
    G0["第1章 全体像"] --> G1["第2章 テスト容易性"]
    G1 --> G2["第3章 ポートアダプタ"]
    G2 --> G3["第4章 Google Test"]
    G3 --> G4["第5章 FFF"]
    G4 --> G5["第7章 AI×TDD"]
    G5 --> G6["第8章 状態遷移"]
    G6 --> G7["第9章 ISR"]
    S0["第6章 環境構築・ビルド"] -. 補助 .-> G3

    style G0 fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
...
温度監視システムを例に学びます。前半では純粋関数、HAL、ポートアダプタを整理し、後半で同じ題材を状態遷移や割り込みの具体例へ発展させます。
