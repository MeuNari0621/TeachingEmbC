...
**前提1** について補足します。AI（大規模言語モデル）は、学習データの統計的パターンに基づいてコードを生成します。つまり「この文脈で最も確率が高いトークン列」を選択する仕組みです。その結果、一見正しそうに見えるが微妙にロジックが間違っているコードや、ある環境では動くが別の環境では動かないコードが生成されることがあります。特に、組み込み特有の制約（型サイズ、エンディアン、リソース制限）はAIの学習データに十分含まれていない可能性があり、誤りのリスクが高まります。
...
> **補足**: もちろん、単純なコード（例: getter/setter）ではTDDのオーバーヘッドが見合わないこともあります。重要なのは「ビジネスロジック」や「複雑な条件分岐」に対してTDDを適用することです。
